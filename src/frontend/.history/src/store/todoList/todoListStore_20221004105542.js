...
const actions = {
  ...
  async addTodoAct({commit}, payload) { // action에서는 데이터를 푸시하고 삭제하는 기능만!!
    const addRes = await axios.instance({
      ...
      console.debug("update",addRes.data.data);
  ...
  async updateTodoAct({commit}, payload) {
    ...
    console.debug("updateRes : ",updRes);
    commit('updateListDataTodo', updRes.data.data.todo);
  ...
  async loadTodoAct({ commit }) {  
    ...
    console.debug("loadTodo", loadRes.data.data);
...
const mutations = {
  ...
  removeListData : (state) =>{
    ...
    const delList = [];
    for(const idx of state.chkDelIdx){
      delList.push(list[idx].key);
    }


    for(let i = 0; i < state.todoList; i++){
      for(const todo of delList)
        if(list[i].key === todo)
        {
          list.splice(i,1);
        }
